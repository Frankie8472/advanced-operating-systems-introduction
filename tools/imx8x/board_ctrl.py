...
def get_usb_backend():
    if sys.platform == "win32":
        # On Windows we can use libusb1 in combination with usbdk
        # https://github.com/libusb/libusb/wiki/Windows#Driver_Installation
        import usb.backend.libusb1
        libusb_path = os.environ.get("LIBUSB_PATH")
        backend = usb.backend.libusb1.get_backend(find_library=lambda x: libusb_path)
        if backend is None:
            print("Couldn't find libusb backend. Try setting LIBUSB_PATH environment variable.", file=sys.stderr)
            return None
        # PyUSB doesn't expose libusb option to use usbdk
        # Workaround: https://github.com/pyusb/pyusb/issues/200#issuecomment-392621726
